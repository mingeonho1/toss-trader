...
def test_cost_from_commissions() -> None:
    # commissionRate는 '소수 비율'(0.0025=25bps, 0.001=10bps)이며 각 행은 [startDate, endDate]
    # 유효기간을 가진다. 오늘을 포함하는 행(=라이브 실요율)을 선택; 없으면 표준요율로 폴백.
    today = date(2026, 9, 23)
    commissions = [
        {"marketCountry": "KR", "commissionRate": "0.00015", "startDate": "2026-01-01", "endDate": None},
        {"marketCountry": "US", "commissionRate": "0.001", "startDate": None, "endDate": "2026-06-30"},   # 과거행(만료)
        {"marketCountry": "US", "commissionRate": "0.0025", "startDate": None, "endDate": None},           # 오늘 포함 → 선택
    ]
    cm = CostModel.from_commissions(commissions, market="US", today=today)
    assert abs(cm.commission_bps - 25.0) < 1e-9, cm.commission_bps   # 라이브 우선: 합성 0.0025행 선택
    cm_promo = CostModel.from_commissions(commissions, market="US", today=date(2026, 3, 1))
    assert abs(cm_promo.commission_bps - 10.0) < 1e-9, cm_promo.commission_bps  # 0.001행 선택 → 10bps
    # 오늘을 포함하는 행이 없으면 표준요율 폴백(0.1%=10bps, 2025-12-01~)
    ...
    assert abs(cm_fb.commission_bps - 10.0) < 1e-9, cm_fb.commission_bps
    print("  ✓ CostModel.from_commissions 통과 (ratio→bps, 유효기간 선택, 폴백=표준 0.1%)")
