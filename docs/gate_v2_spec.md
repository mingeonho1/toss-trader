...
## 부록 v2.1 — 사이클 3 이전 사전확정 (2026-09-23)

사이클 2 결과를 보고 규칙을 '구제용'으로 바꾸지 않기 위해, 다음 보정은 **사이클 3 결과를 보기 전에** 확정한다.

1. **상대 리스크 트랙.** 벤치마크 자체가 −50% 캡을 위반하는 경우(NDX 1x 설계구간 MDD −83%),
   절대 캡은 "항상 투자" 계열을 구조적으로 배제한다. 이때 리스크 축은 다음을 **설계·홀드아웃 모두**에서 만족하면 통과:
   `MDD_s ≥ MDD_b − 5%p` 그리고 `Ulcer_s ≤ 1.10 × Ulcer_b`. 절대 캡(−50%)은 벤치가 캡을 지키는 구간/레인에서 그대로 유지.
   레버리지 사용 전략은 추가로 전표본 `MDD_s ≥ −70%`(파산 방지 하한).
2. **현금 수익률 기본값 0%.** 토스 USD 예수금 이자는 미확인 → 유휴 현금 수익률 0%가 기본, DTB3는 민감도로만 보고.
   BIL/SGOV로 현금을 대체하는 경우 매매비용(25bp/side)을 반드시 부과.
3. **프로그램 전체 N.** DSR의 N은 원장 전체 시도 수(아이디어 간 포함)의 N_eff로 계산해 함께 보고. 아이디어 내부 N만 쓴 DSR은 참고치.
4. **신호 재사용 페널티.** 이미 홀드아웃을 본 신호를 새 실행 방식(레버리지 슬리브·코어-위성 등)으로 재시험하면
   새 idea_id를 쓰되 홀드아웃은 "반오염"으로 표기하고, 통과 기준을 RC/SPA p < 0.01로 강화한다.
5. **코어-위성 평가.** 위성 전략은 단독이 아니라 `코어(QQQ) + 위성` 포트폴리오 vs QQQ 100%로 판정한다
   (위성이 현금에 머무는 기회비용 포함).

---

## 부록 v2.2 — 프로그램 DSR 추정기 버그픽스 (2026-09-23)

> **이것은 판정 임계(threshold) 변경이 아니라 추정기(estimator) 정정이다.** DSR≥0.95 통과·0.90–0.95 경계 등 모든 결정 문턱은 부록 v2.1과 동일하게 유지된다. `reports/cycle3_c3c_pit.md`·`reports/cycle4_c4b_fee_reeval.md`가 지적한 "모든 전략의 프로그램 DSR ≈ 0.00" 아티팩트를 고친다. 재검증: `reports/cycle5_dsr_recheck.md`. 원장 이력은 되쓰지 않으며(사양 §3.1 append-only), 사후 플래그는 사이드카 `reports/trials_ledger_flags.json`에만 적재한다.

### 1. 근본원인 (원장 아티팩트)

프로그램 DSR의 우연-최대-Sharpe 문턱은 `SR*₀ = sqrt(V)·[(1−γ)Φ⁻¹(1−1/N) + γΦ⁻¹(1−1/(N·e))]`로, **시도 SR 표본분산 V**에 정비례한다. 원장(`trials_ledger.jsonl`, 503행/73아이디어)에는 **퇴화(무낙폭 현금성) 시도** 3행(`c2b_vix_1x/2x/3x`, `mult=1.95`)이 있는데 **MDD=0·Ulcer=0·일 SR=2.2767**(연율 ~36.1)이다. VIX 트리거 슬리브가 사실상 상시 현금에 머물러 자본곡선이 단조증가 → 실현변동성≈0 → |SR|이 폭증한 것으로, **단위오류가 아니다**(원장 전 행에서 `sr_annual == sr_daily·√252`가 정확히 성립함을 확인). 이 3행이 V를 `0.00063 → 0.0317`로 ~50배 부풀려 `SR*₀`를 **일 0.43(연율 ~6.8)**로 올렸고, 어떤 실전략(일 SR ~0.05–0.09)도 이를 넘지 못해 DSR이 0.00으로 붕괴했다.

Bailey–López de Prado에서 DSR의 **시도집합 = 같은 데이터에 시도한 구성(configuration)**이다. 이 정의에 어긋나는 행들이 V·N을 오염시켰다: (a) 홀드아웃/WF(사후평가, 시도 아님), (b) 레인3 거래단위 행(연단위 SR 부적용, §3.5 — T=거래수, SR 단위 불일치), (c) 관측수 부족 행, (d) 퇴화 저변동성 행.

### 2. 정정 (`gate.py`, 하위호환 · 선택적 인자)

세 가지를 도입하되 기본 동작은 보존한다. `dsr_program(..., robust=True)`·`gate_eval.run_splits(..., program_dsr_robust=True)`로 opt-in.

**(a) 시도풀 필터 `filter_trial_records`.** 기본으로 `splits={"design"}`(비설계 제외), `exclude_lanes={3}`(거래레인 제외), `min_obs=252`(관측 부족 제외), `min_abs_mdd=1e-9`(무낙폭 현금성 제외: `|MDD|<1e-9` 또는 `MDD=Ulcer=0`), 그리고 사이드카/레코드 플래그(`benchmark`·`diagnostic`·`degenerate`·`trade_lane`·`cash_like`) 제외. 각 인자에 `None`을 주면 해당 필터 해제.

**(b) 로버스트 V `robust_sr_variance`.** MAD 기반 척도로 윈저화 후 표본분산:

```
σ_MAD = 1.4826 · median(|SRᵢ − median(SR)|)        # 정규분포에서 σ의 일치추정
SRᵢ  ← clip(SRᵢ, median(SR) ± 5·σ_MAD)             # 5σ 윈저화(문서화된 컷)
V     = 표본분산(ddof=1) of 윈저화된 SRᵢ
```

MAD=0(동률 과다)이면 고전 표준편차로 폴백. `V_raw`(비로버스트)와 `n_winsorized`를 함께 보고해 정직성을 유지한다. (필터가 퇴화행을 이미 빼므로 윈저화는 **잔여** 꼬리에 대한 이중 방어다.)

**(c) 아이디어 레벨 N_eff `idea_cluster_n_eff`.** 각 `idea_id`를 그 config들의 **SR 평균(대표값)** 1개로 접어 `N_eff = 고유 아이디어(대표) 수`. 한 아이디어의 그리드 이웃(lookback 200 vs 201 …)이 N을 부풀려 DSR을 과도하게 엄격하게 만드는 것을 막는다(§3.2의 상관 클러스터를 아이디어 레벨로 승격; 대표 스트림 저장 시 상관 그리디로 추가 병합 가능, 미저장 원장은 대표 수로 보수적 폴백).

DSR 자체는 `PSR(SR_hat, SR*₀ = expected_max_sharpe(N_eff, V))`로 부록 v2.1과 동일하게 조립한다.

### 3. 원장 전체 재계산 결과 (요지)

| 추정기 | 시도풀 | N_eff | V | SR*₀(일) |
|---|---:|---:|---:|---:|
| 구(버그) | 503행 전부 | 73 | 0.03172 | **0.4306** |
| 신(v2.2) | 424 설계시도(퇴화·레인3·단기 제외, 11 윈저) | 70 | 0.00063 | **0.0603** |

정정 후에도 **홀드아웃(게이트 판정 축)에서 DSR≥0.90에 도달하는 아이디어는 0개**(최고 `c2d_resid_mom_top3` 0.81)이므로 **`gate.decide` 판정은 하나도 바뀌지 않는다**(전부 FAIL 유지). 즉 이 픽스는 0.00이라는 **비판별 아티팩트를 판별 가능한 값(0.4–0.8)으로 되살릴 뿐**, 채택 결론을 완화하지 않는다. 상세 표·정직성 주석: `reports/cycle5_dsr_recheck.md`.
